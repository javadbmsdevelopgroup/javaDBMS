...
public class TranstionFunc {
    ...
    int transition(AutomatonNode from,List<InputItem> input){
        ...
        for(InputItem s:input) {
            ...
            for(int i=0;i<tifs.size();i++){
                if(tifs.get(i).from.nodeCode!=cur.nodeCode) continue;
                //转移类型是关键字
                if(tifs.get(i).type== TransitionType.KEYWORD){
    ...
    AutomatonNode transition(AutomatonNode from,InputItem input){
        AutomatonNode cur=from;


        for(int i=0;i<tifs.size();i++){
            if(tifs.get(i).from.nodeCode!=cur.nodeCode) continue;   //遍历转移函数列表，找到相应的转移函数
